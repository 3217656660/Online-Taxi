 ...
 * 只有请求头中带有token符合要求才可以访问,是第一层拦截，用于屏蔽无效请求。
 * 第二层拦截的检验用对应业务的注解实现。
 */
//@Component
@Slf4j
...
public class TokenGatewayFilter implements GlobalFilter{
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        String mobile = exchange.getRequest().getHeaders().getFirst("mobile");
        log.info("mobile={}", mobile);

        String tokenKey = mobile + ":";
        log.info("tokenKey={}", tokenKey);

        boolean b = redisUtil.hasKey(tokenKey);
        log.info("b={}", b);

        Object mobileObj = redisUtil.get(tokenKey);
        log.info("mobileObj={}", mobileObj);
